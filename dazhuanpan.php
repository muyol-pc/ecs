...
if ($action == 'index')
{
    ...
    if ($fans == false) {
        ...
        //增加人数，和浏览次数
        $db->autoExecute($ecs->table('bigwheel_reply'),array('fansnum' => $reply['fansnum'] + 1, 'viewnum' => $reply['viewnum'] + 1),"UPDATE",'id='. $reply['id']);
    } else {
        //增加浏览次数
        $db->autoExecute($ecs->table('bigwheel_reply'), array('viewnum' => $reply['viewnum'] + 1),"UPDATE",'id='. $reply['id']);
    }
    //判断是否获奖
    $condition="";
    if(!empty($fansID)){
...
//todo
    ...
    $awards = $db->getAll("SELECT * FROM " . $ecs->table('bigwheel_award') . " WHERE " ." rid = " . $id . " order by id desc");
    $condition="";
    ...
    if (empty($fans)) {
        ...
        $db->autoExecute($ecs->table('bigwheel_fans'),$fans,"INSERT");
    ...
    //当天抽奖次数
    $db->autoExecute($ecs->table('bigwheel_fans'), array('todaynum' => $fans['todaynum'] + 1, 'last_time' => $last_time),"UPDATE","id=".$fans['id']);

    //总抽奖次数
    $db->autoExecute($ecs->table('bigwheel_fans'), array('totalnum' => $fans['totalnum'] + 1),"UPDATE","id=".$fans['id']);
    $gifts = array(
    ...
    if ((!empty($prizetype)) && ($fans['awardnum'] < $reply['award_times'])) {
        //中奖
        $sn = random(16);
        $db->autoExecute($ecs->table('bigwheel_reply'),array('c_draw_' . $prizetype => $reply['c_draw_' . $prizetype] + 1),"UPDATE","id=".$reply['id']);
        //保存sn到award中
        $insert = array(
            'rid' => $id,
            'fansID'=>$user_id,
    ...
    $fansID =$user_id;
    $from_user = $user_name;
    $fans = $GLOBALS['db']->getRow("SELECT id FROM " . $GLOBALS['ecs']->table('bigwheel_fans') . " WHERE rid = " . $id . " and fansID=" . $fansID . " and from_user='" . $from_user . "'");
    if ($fans == false) {
        $data = array(
            'success' => 0,
            'msg' => '保存数据错误！',
        ...
        $temp = $GLOBALS['db']->autoExecute( $GLOBALS['ecs']->table('bigwheel_fans'), array('tel' => $_REQUEST['tel']),'UPDATE',"rid=".$rid." and fansID=".$fansID);
