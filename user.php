...
//        $smarty->display('user_passport.dwt');
    ...
    else
    {
        ...
        exit;*/
        $mobile_phone = $_POST['mobile_phone'];//手机号
        $captcha      = $_POST['captcha'];//验证码
        $company      = $_POST['company'];//公司名称
        $user_name    = $_POST['user_name'];//姓名
        $category     = $_POST['category'];//分类id
        $reg_time = time();//时间
        //正则验证手机号
        if(!preg_match('/^(1(([35][0-9])|(47)|[8][0126789]))\d{8}$/',$mobile_phone)){
            $return['state'] = 400;
            $return['msg']  = "手机号码不正确！";
            echo json_encode($return);
            exit;
        ...
        //检查是否已存在
        $selectResult = $db->getAll("SELECT mobile_phone FROM ". $ecs->table('users')." WHERE mobile_phone=".$mobile_phone);
        if($selectResult){
            $return['state'] = 400;
            $return['msg']   = "当前用户手机号已注册！";
            echo json_encode($return);
            exit;
        }
        //写入数据库
...
elseif ($action == 'act_login')
{
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        //检测是否通过审核
        /*$checkResult = $db->getOne("SELECT state FROM ". $ecs->table('users') ." WHERE user_name='".$username."' AND password='".$password."'");
        if($checkResult['state'] == 0){
            $return['state'] = 400;
            $return['msg'] = "登录失败，您的账号未通过审核，请联系管理员！";
            echo json_encode($return);
            exit;
        }else{
            $return['state'] = 200;
