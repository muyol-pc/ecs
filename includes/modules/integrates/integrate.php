 ...
 * @return bool
 */
public function is_phone($user_email) {
 ...
 * 检测审核是否通过,用户是否存在
 */
public function is_membernone($username){
        // 用户名是手机格式
            if ($this->is_phone($username)) {
                $sql= "select user_id from ecs_users where mobile_phone = '$username'";
                 $sql2= "select state from ecs_users where mobile_phone = '$username'";
            }else{
                //用户名是普通字符串格式
                $sql= "select user_id from ecs_users where user_name = '$username'";
                $sql2= "select state from ecs_users where user_name = '$username'";
            }
            $real=$this->db->getOne($sql,true);
            $re_state = $this->db->getOne($sql2,true);
            if (empty($real)) {
                return false;
            }elseif (!$re_state) {
                return false;
